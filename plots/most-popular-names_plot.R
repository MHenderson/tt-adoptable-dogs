...
p <- top_fifty_names %>%
  ...
  labs(
    ...
    title = NULL,
    subtitle = NULL,
    caption = "Graphic: Matthew Henderson"
  ) +
  theme_minimal() +
  theme(
  ...
  annotate(geom = "text", x = 40, y = 125, label = "Data collected from the PetFinder API\nby Amber Thomas for all adoptable dogs\nin each state on September 20, 2019.", size = 6, hjust = 0, family = f1)
